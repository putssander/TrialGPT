...
AISuite config Ollama on docker host machine:
...
### Maastro Datasets edit
Use conversion script to convert xlsx to jsonl [1 corpus_conversion](TrialGPT/notebooks/1 corpus_conversion.ipynb)
...
### Maastro edit
Use conversion script to convert patient pdfs to queries.jsonl [2 pdf_to_queries_ocr](TrialGPT/notebooks/2 pdf_to_queries_ocr.ipynb)

Set Ollama when not default
[OLLAMA_API_URL](https://github.com/andrewyng/aisuite/blob/main/aisuite/providers/ollama_provider.py)

```bash
export OLLAMA_API_URL="http://host.docker.internal:11434"
# --> aisuite ollama client
# export OLLAMA_API_BASE_URLL="http://host.docker.internal:11434" --> ollama client
```

```bash
python trialgpt_retrieval/keyword_generation.py mstro ollama:phi4
...
### Maastro edit
> The creation process of the `id2queries.json` file was previously unclear, as only its input was available without any documented output.  
> To address this, the new notebook `id2query.ipynb` was introduced to convert the output of the `keyword_generation.py` script into the correct `id2queries.json` format.  
> Additionally, the hybrid fusion script has been updated to incorporate keywords generated by Ollama.

```bash
python trialgpt_retrieval/hybrid_fusion_retrieval.py mstro ollama:phi4 20 1 1
```
...
this becomes:
python trialgpt_matching/run_matching.py mstro ollama:phi4
etc...
```
...
maastro edit, a required file dataset_info.json does not excist but can be generated using notebook ...
run
python trialgpt_ranking/run_aggregation.py mstro ollama:phi4 results/matching_results_mstro_ollama:phi4.json
...
python trialgpt_ranking/rank_results.py results/matching_results_mstro_ollama:phi4.json results/aggregation_results_mstro_ollama:phi4.json
